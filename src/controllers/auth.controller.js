...
export const login = async (req, res) => {
  try {
    ...
    // ✅ 4) Update LAST LOGIN TIME (ONLY ON SUCCESS)
    await pool.query(
      "UPDATE users SET last_login_at = NOW() WHERE u_id = ?",
      [user.u_id]
    );
    ...
    const safeUser = {
      ...
      role: user.u_role,
      u_region: user.u_region,
      last_login_at: new Date(), // optional frontend use
    };
